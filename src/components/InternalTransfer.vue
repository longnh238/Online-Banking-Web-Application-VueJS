    ...
    <div class="content">
      ...
      <form id="transferForm">
        ...
        <div class="form-transactions">
          ...
          <select id="toAccount" name="toAccount" v-model="destinationAccountIndex"
            :disabled="!destinationAccounts.length">
            <option v-for="(destinationAccount, index) in destinationAccounts" :key="index" :value="index">
              {{ `Account ${destinationAccount.id}
              (${destinationAccount.type.charAt(0)}${destinationAccount.type.slice(1).toLowerCase()}) - Balance:
              ${destinationAccount.balance} CAD` }}
            </option>
          </select>
        </div>
        <div class="form-transactions">
...
export default ({
  ...
  watch: {
    sourceAccountIndex(newIndex) {
      if (newIndex !== '') {
        this.destinationAccounts = this.accounts.filter(account => account.id !== this.accounts[newIndex].id)
        this.destinationAccounts.sort((a, b) => a.id - b.id);
      }
    }
  },
  methods: {
    ...
    transferBetweenAccounts(event) {
      ...
      this.transferRequest.destinationAccountId = this.destinationAccounts[this.destinationAccountIndex].id;
...
label {
  ...
  font-weight: bold;
  color: #2980b9;
  /* Changed label color to match the theme */
}
